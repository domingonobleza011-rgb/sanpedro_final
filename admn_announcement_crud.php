...
<button type="button" class="btn-perma-delete"
        data-id="<?= htmlspecialchars($row['id_announcement'], ENT_QUOTES) ?>"
        data-preview="<?= htmlspecialchars(str_replace(["\r\n","\r","\n"], ' ', $row['event']), ENT_QUOTES) ?>"
        onclick="event.stopPropagation(); openDeleteModal(this.dataset.id, this.dataset.preview)">
